    ...
    <header :style="{ backgroundColor: $store.state.theme }">
      ...
      <div class="userinfo">
        <el-dropdown trigger="hover"  @command="logout">
          <el-avatar :icon="UserFilled" class="avatar">{{$store.state.name}}</el-avatar>
          <template #dropdown>
            <el-dropdown-menu>
    ...
    <aside class="nav">
      ...
      >
        <template v-for="item in $router.options.routes" :key="item.name">
          <el-sub-menu
          :index="item.path"
          v-if="(!item.hidden && item.children && item.children.length > 0 && item.isPermisson?.includes($store.state.roleType))"
          >
            <template #title>
              <el-icon>
          ...
          <el-menu-item :index="item.path" v-else-if="(item.isPermisson?.includes($store.state.roleType) && !item.hidden)">
            <el-icon>
              <component :is="item.icon"></component>
            </el-icon>
...
export default {
  ...
  setup () {
    ...
    const user = ref('')
    const pwd = ref('')
    const logout = async (com) => {
      if (com === 'logout') {
        // store.commit('changeUserName', { username: '', roleType: 1 })
        store.commit('changeUser', await login({ _username: 'super', _pwd: '123456' }))
        return router.push('/')
      }
    }
    const login = async ({ _username, _pwd } = {}) => {
      const user = _username ?? 'admin'
      const pwd = _pwd ?? '123456'
      const para = {
        username: user,
        password: pwd
      }

      return await loginApi.postLoginData(para)
    }
    onMounted(async () => {
      store.commit('changeUser', await login())
    })
    return {
      logout,
      user,
