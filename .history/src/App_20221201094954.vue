...
<script>
import { Compass, Management, Document, Histogram, List, UserFilled } from '@element-plus/icons-vue'
...
export default {
  ...
  setup () {
    ...
    const login = () => {
      let user = LoginUsers.filter(roleType => roleType === 1).username
      let pwd = LoginUsers.filter(roleType => roleType === 1).password
      const para = {
        username: user,
        password: pwd
      }
      userApi.postLoginData(para).then((res) => {
        console.log(res)
        user = res.LoginUsers.username
        pwd = res.LoginUsers.password
      })
    }
